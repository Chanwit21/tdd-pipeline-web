...
export default function MasterConfigPage() {
  ...
  return <div className="stack">
      ...
      <div className="subpanel">
        {sub === "dept" && <>
          <div className="table-wrap"><table><thead><tr><th className="col-no">No.</th><th>Department</th><th>ชื่อเต็ม</th><th>Deal Owner (Lock)</th><th>Manager ที่ผูก</th><th /></tr></thead><tbody>
            {config.departments.map((d,i)=><tr key={d.id}><td className="col-no">{i+1}</td><td className="cell-strong">{d.code}</td><td>{d.name}</td><td>{d.defaultOwner || "—"}</td><td>{users.filter(u=>u.departmentId===d.id && u.role==="MANAGER" && u.active).map(u=>u.fullName).join(", ") || "—"}</td><td><button className="rowbtn" title={`แก้ไข ${d.code}`} aria-label={`แก้ไข ${d.code}`} onClick={()=>open("departments",d)}><IcoEdit size={13} /></button></td></tr>)}
          </tbody></table></div>
    ...
    <Modal open={!!form} onClose={()=>{if(!busy)setForm(null);}} width={560}>
      {form && <><div className="modal-head"><h3>{form.id?"แก้ไข":"เพิ่ม"} {title[form.kind]}</h3><button className="icon-x" aria-label="ปิด" disabled={busy} onClick={()=>setForm(null)}><IcoClose size={15} /></button></div><div className="mpanel">
    ...
    <Modal open={!!pendingDelete} onClose={()=>{if(!busy)setPendingDelete(null);}} width={480}>
      <div className="modal-head"><h3>ยืนยันลบ {pendingDelete?.name}</h3></div><div className="mpanel">ลบได้เฉพาะรายการที่ไม่มีข้อมูลอ้างอิง{error&&<div className="warn-banner" role="alert">{error}</div>}</div><div className="modal-foot"><button className="btn" disabled={busy} onClick={()=>setPendingDelete(null)}>ยกเลิก</button><button className="btn btn-danger" disabled={busy} onClick={remove}>ยืนยันลบ</button></div>
    </Modal>
  </div>;
}
