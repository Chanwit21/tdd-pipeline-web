  ...
  return <div className="stack">
    ...
    <div className="panel">
      ...
      <div className="subpanel">
        {sub === "dept" && <>
          <div className="master-actions" style={{justifyContent:"flex-end",marginBottom:10}}><button className="btn btn-primary btn-sm" onClick={()=>open("departments")}>+ เพิ่มแผนก</button></div>
        ...
        {sub === "prob" && <>
          ...
          <div className="subsection-head" style={{marginTop:22}}>Deal Stage ↔ Probability — กฎที่ต้องตรงกัน</div>
    ...
    <Modal open={!!form} onClose={()=>{if(!busy)setForm(null);}} width={560}>
      {form && <><div className="modal-head"><h3>{form.id?"แก้ไข":"เพิ่ม"} {title[form.kind]}</h3><button className="icon-x" aria-label="ปิด" disabled={busy} onClick={()=>setForm(null)}>×</button></div><div className="mpanel">
        ...
        <div className="form-grid" style={{gridTemplateColumns:"1fr"}}>
          ...
          <div className="form-field"><label>{form.kind==="departments"?"ชื่อเต็ม":"ชื่อ"} <span className="req">*</span></label><input maxLength={form.kind==="departments"?100:form.kind==="stages"?80:50} value={form.name} onChange={e=>setForm({...form,name:e.target.value})} /></div>
